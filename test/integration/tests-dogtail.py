...
def test_preferences_structure():
    ...
    assert_eq("Monitors page tab exists", str("Monitors" in tab_names), "True")
...
# ---------------------------------------------------------------------------
# Test: Per-Monitor Window Size Constraints (gsettings roundtrip)
#
# The monitor controls in the prefs UI use a Gtk.DrawingArea with Cairo
# rendering for the monitor layout graphic.  In headless mode there is no
# pointer device, so clicking individual monitor rectangles is not possible
# via AT-SPI.  Instead we verify gsettings reads and writes directly.
# ---------------------------------------------------------------------------


def test_monitor_constraints():
    """Verify monitor-constraints gsettings key read/write roundtrip."""
    print("\n-- Per-Monitor Window Size Constraints --")

    # Default should be empty array
    default_val = gsetting_get("monitor-constraints")
    assert_eq("default monitor-constraints is empty", default_val, "@a(suubb) []")

    # Write a single constraint entry and read it back
    entry = '@a(suubb) [(\'DP-1\', 1920, 1080, true, false)]'
    gsetting_set("monitor-constraints", entry)
    result = gsetting_get("monitor-constraints")
    assert_eq(
        "single constraint stored",
        result,
        entry
    )
    ...
    two_entries = (
        "@a(suubb) "
        "[(\'eDP-1\', 3440, 1440, false, false), "
        "(\'DP-1\', 1920, 1080, true, true)]"
    )
    gsetting_set("monitor-constraints", two_entries)
    result = gsetting_get("monitor-constraints")
    assert_eq(
        "two constraints stored",
        result,
        two_entries
    )

    # Reset to empty
    gsetting_set("monitor-constraints", "@a(suubb) []")
    result = gsetting_get("monitor-constraints")
    assert_eq("constraints cleared", result, "@a(suubb) []")

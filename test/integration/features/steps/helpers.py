...
# ---------------------------------------------------------------------------
# D-Bus agent helpers (org.gnome.Shell.AnvilTest)
# ---------------------------------------------------------------------------


def call_agent(method, param=""):
    cmd = (
        f"gdbus call --session "
        f"--dest {SERVICE_NAME} "
        f"--object-path {OBJECT_PATH} "
        f"--method org.gnome.Shell.AnvilTest.{method}"
    )
    if param:
        cmd += f" '{param}'"
    ...
    if rc != 0 or not out:
        return ""
    out = out.strip()
    # Strip D-Bus string tuple wrapper: ('value',) -> value
    m = re.match(r"^\('(.*?)',?\)$", out)
...
def get_extension_errors():
    cmd = (
        f"gdbus call --session "
        f"--dest org.gnome.Shell "
        f"--object-path /org/gnome/Shell "
        f"--method org.gnome.Shell.Extensions.GetExtensionErrors "
        f"'{UUID}'"
    )
    rc, out, _ = run_shell(cmd)
    if rc != 0:
...
def eval_test_state(expr):
    raw = call_agent("GetTestState")
    if not raw:
        return "null"
    try:
        import json
        data = json.loads(raw)
        val = data.get(expr, "null")
        if val is True:
            return "true"
        if val is False:
            return "false"

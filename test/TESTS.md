...
> **Testing approach:** Three strategies run in CI and locally:
...
> 2. **Behave BDD + Dogtail/AT-SPI** (10 scenarios, 46 steps in `test/e2e/features/`) —
>    preferences dialog structure, switch state verification, page tab navigation.
>    Produces an HTML report with AT-SPI tree snapshots on failure.
> 3. **Unit tests** (vitest, 182 tests) — pure logic, tree operations, utilities.
>
> Tests run across Fedora 42 (GNOME 48), 43 (GNOME 49), and 44 (GNOME 50).
...
- [ ] Should open prefs.js via `<Super> + Period` in GNOME 3.3x+.
  - ❌ **Not testable** — GNOME Shell 50 / mutter 50.1 removed the `--nested` flag, making keyboard injection (xdotool/wtype) impossible. The D-Bus `OpenExtensionPrefs` method is used instead.
...
- [x] Switch widget `.checked` state matches gsettings for 7 mapped switches.
  - ✅ Verified via Behave/Dogtail: `preferences.feature` — `Scenario Outline: Switch "<name>" checked state matches gsetting`, toggles via gsettings and re-verifies `.checked`. Navigates to the correct page tab before searching.
...
- [x] Tabbed Focus Hint updates can be reset
  ...
  **Implementation notes:**
  ...
  - ❌ Visual rendering (color on screen, preview hints) cannot be verified in a headless container.
...
- [ ] Should show a preview hint where the window would be tiled.
  - ❌ **Not testable** — drag-and-drop requires mouse pointer device simulation; no virtual pointer protocol is available in headless compositor.
...
- [ ] On dropping to a different monitor, should tile based on the preview hint position shown unless empty monitor.
  - ❌ **Not testable** — single virtual monitor only.
...
- [ ] Visual verification of floating window layer position.
  - ❌ **Not testable** — requires pixel-level rendering comparison.
...
- [ ] when moving the focus to another window with keyboard or mouse.
  - ⚠️ **Partially testable**: `move-pointer-focus-enabled` setting can be toggled (verified in `test_focus_pointer`). Actual pointer position cannot be queried via D-Bus in headless mode.
...
- [ ] when alternating to a new window using `Alt+Tab` or `Super+Tab`.
  - ❌ **Not testable** — keyboard simulation not available.

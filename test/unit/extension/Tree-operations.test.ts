...
describe("Tree Operations", () => {
  ...
  describe("next", () => {
    it("should find next sibling to the right", () => {
      ...
      const next = ctx.tree.next(node1, Meta.MotionDirection.RIGHT);
    ...
    it("should find next sibling to the left", () => {
      ...
      const next = ctx.tree.next(node2, Meta.MotionDirection.LEFT);
    ...
    it("should find next sibling downward", () => {
      ...
      const next = ctx.tree.next(node1, Meta.MotionDirection.DOWN);
    ...
    it("should find next sibling upward", () => {
      ...
      const next = ctx.tree.next(node2, Meta.MotionDirection.UP);
    ...
    it("should return null for node at end", () => {
      ...
      const next = ctx.tree.next(node, Meta.MotionDirection.RIGHT);
    ...
    it("should navigate across different orientations", () => {
      ...
      const next = ctx.tree.next(container, Meta.MotionDirection.RIGHT);
  ...
  describe("move", () => {
    ...
    it("should append to currentMonWsNode when no next node", () => {
      const { monitor } = getWorkspaceAndMonitor(ctx);
  ...
  describe("next - Stacked Container Navigation", () => {
    it("should cycle through windows in stacked container using UP/DOWN", () => {
      ...
      const nextFromFirst = ctx.tree.next(node1, Meta.MotionDirection.DOWN);
      expect(nextFromFirst).toBe(node2);

      // Navigate down from second window
      const nextFromSecond = ctx.tree.next(node2, Meta.MotionDirection.DOWN);
    ...
    it("should navigate up in stacked container", () => {
      ...
      const prev = ctx.tree.next(node2, Meta.MotionDirection.UP);
    ...
    it("should exit stacked container when navigating left/right", () => {
      ...
      const nextRight = ctx.tree.next(node1, Meta.MotionDirection.RIGHT);
  ...
  describe("next - Tabbed Container Navigation", () => {
    it("should cycle through tabs using LEFT/RIGHT", () => {
      ...
      const nextFromFirst = ctx.tree.next(node1, Meta.MotionDirection.RIGHT);
      expect(nextFromFirst).toBe(node2);

      // Navigate right from second tab
      const nextFromSecond = ctx.tree.next(node2, Meta.MotionDirection.RIGHT);
      expect(nextFromSecond).toBe(node3);
    });
    ...
    it("should navigate left between tabs", () => {
      ...
      const prev = ctx.tree.next(node2, Meta.MotionDirection.LEFT);
      expect(prev).toBe(node1);
    });
    ...
    it("should exit tabbed container when navigating up/down", () => {
      ...
      const nextDown = ctx.tree.next(node1, Meta.MotionDirection.DOWN);

...
export class PreferencesPage extends Adw.PreferencesPage {
  ...
  add_group({ title, description = "", children, header_suffix = "" }) {
    ...
    if (header_suffix) group.set_header_suffix(/** @type {any} */ (header_suffix));
    this.add(group);
    return group;
  }
}
...
export class SpinButtonRow extends Adw.ActionRow {
  ...
  constructor({
    ...
    gspin.xalign = 1;
    if (bind && settings) {
      /** @type {any} */ (settings).bind(bind, gspin, "value", Gio.SettingsBindFlags.DEFAULT);
    } else if (init !== undefined) {
      gspin.value = init;
      gspin.connect("value-changed", (widget) => {
        if (onChange) /** @type {any} */ (onChange)(widget.value);
      });
    }
...
export class DropDownRow extends Adw.ActionRow {
  ...
  constructor({ title, settings, bind, items, subtitle = "", type }) {
    ...
    this.add_suffix(
      new ResetButton(/** @type {any} */ ({ settings, bind, onReset: () => this.reset() }))
    );
  }
...
export class ClearButton extends Gtk.Button {
  ...
  constructor({ _settings = undefined, _bind = undefined, onClear }) {
    super({

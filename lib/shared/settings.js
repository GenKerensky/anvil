...
export class ConfigManager extends GObject.Object {
  ...
  loadFileContents(configFile) {
    ...
    if (success) {
      const stringContents = new TextDecoder().decode(/** @type {Uint8Array} */ (contents));
  ...
  get windowProps() {
    ...
    if (success) {
      const windowConfigContents = new TextDecoder().decode(/** @type {Uint8Array} */ (contents));

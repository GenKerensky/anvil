...
export class Node extends GObject.Object {
  ...
  constructor(type, data) {
    ...
    this.mode = /** @type {string} */ (Window.WINDOW_MODES.DEFAULT);
    this.percent = 0.0;
    this._rect = null;
    this.tab = null;
  ...
  set float(value) {
    if (this.isWindow()) {
      ...
      let floatAlwaysOnTop = this.settings?.get_boolean("float-always-on-top-enabled") ?? false;
      if (value) {
...
export class Tree extends Node {
   ...
   */
  createNode(parentObj, type, value, mode = /** @type {string} */ (Window.WINDOW_MODES.TILE)) {
  ...
  focus(node, direction) {
    ...
    if (metaWindow.minimized) {
      ...
      const metaWin = /** @type {any} */ (previousMetaWindow);
  ...
  nextMonitor(nodeWindow, position, orientation) {
    ...
    const nodeValue = /** @type {any} */ (nodeWindow.nodeValue);
    // Use the built in logic to determine adjacent monitors
    let monitorDirection = Utils.directionFrom(position, orientation);
    let targetMonitor = global.display.get_monitor_neighbor_index(
      nodeValue.get_monitor(),
      monitorDirection
    );
    if (targetMonitor < 0) return targetMonitor;
    let monWs = `mo${targetMonitor}ws${nodeValue.get_workspace().index()}`;
    let monitorNode = this.findNode(monWs);
    return monitorNode;
  }
  ...
  processNode(node) {
    ...
    if (node.nodeType === NODE_TYPES.MONITOR || node.nodeType === NODE_TYPES.CON) {
      ...
      params.sizes = sizes;
      let showTabs = this.settings?.get_boolean("showtab-decoration-enabled") ?? false;

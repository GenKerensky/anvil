...
class SettingsPopupSwitch extends PopupSwitchMenuItem {
  ...
  /** @type {AnvilExtension} extension */
  extension;

  /**
   * @param {string} title
   * @param {AnvilExtension} extension
   * @param {string} bind
   */
  constructor(title, extension, bind) {
    const active = !!(/** @type {any} */ (extension.settings).get_boolean(bind));
    super(title, active);
    this.extension = extension;
    Logger.info(bind, active);
    this.connect("toggled", (item) =>
      /** @type {any} */ (this.extension.settings).set_boolean(bind, item.state)
    );
  }
}
...
export class FeatureMenuToggle extends QuickMenuToggle {
  ...
  constructor(extension) {
    ...
    /** @type {any} */ (this.menu)._settingsActions[this.extension.uuid] = settingsItem;
  }
}

...
export class Keybindings extends GObject.Object {
  ...
  // @deprecated
  _bindSignals() {
    (global.display as Meta.Display).connect("accelerator-activated", (__display, action) => {
   ...
   */
  listenFor(accelerator: string, callback: () => void) {
    const grabFlags = Meta.KeyBindingFlags.NONE;
    const action = global.display.grab_accelerator(accelerator, grabFlags);

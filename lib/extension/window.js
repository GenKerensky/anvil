...
/** @typedef {import('../../extension.js').default} AnvilExtension */
...
export class WindowManager extends GObject.Object {
  ...
  constructor(ext) {
    ...
    this.shouldFocusOnHover = !!(
      /** @type {any} */ (this.ext.settings)?.get_boolean("focus-on-hover-enabled")
    );
  ...
  addFloatOverride(metaWindow, withWmId) {
    const configMgr = /** @type {any} */ (this.ext.configMgr);
  ...
  toggleFloatingMode(action, metaWindow) {
    ...
    if (floatingExempt) {
      ...
      this.windowProps = /** @type {any} */ (this.ext.configMgr).windowProps;
  ...
  _bindSignals() {
    ...
    const extDisplay = /** @type {any} */ (global.display);
    ...
    const extWsm = /** @type {any} */ (global.workspace_manager);
    ...
    let numberOfWorkspaces = extWsm.get_n_workspaces();

    for (let i = 0; i < numberOfWorkspaces; i++) {
      let workspace = extWsm.get_workspace_by_index(i);
      this.bindWorkspaceSignals(workspace);
    }

    let settings = /** @type {any} */ (/** @type {any} */ (this.ext.settings));
  ...
  trackCurrentMonWs() {
    let metaWindow = /** @type {any} */ (this.focusMetaWindow);
  ...
  command(action) {
    ...
    switch (action.name) {
      ...
      case "FocusBorderToggle": {
        let focusBorderEnabled = /** @type {any} */ (this.ext.settings).get_boolean(
          "focus-border-toggle"
        );
        /** @type {any} */ (this.ext.settings).set_boolean(
          "focus-border-toggle",
          !focusBorderEnabled
        );
        break;
      ...
      case "TilingModeToggle": {
        ...
        let tilingModeEnabled = /** @type {any} */ (this.ext.settings).get_boolean(
          "tiling-mode-enabled"
        );
        /** @type {any} */ (this.ext.settings).set_boolean(
          "tiling-mode-enabled",
      ...
      case "GapSize": {
        let gapIncrement = /** @type {any} */ (this.ext.settings).get_uint(
        ...
        /** @type {any} */ (this.ext.settings).set_uint("window-gap-size-increment", gapIncrement);
        break;
      }
      case "WorkspaceActiveTileToggle": {
        let activeWorkspace = global.workspace_manager.get_active_workspace_index();
        let skippedWorkspaces = /** @type {any} */ (this.ext.settings).get_string(
          "workspace-skip-tile"
        );
        ...
        /** @type {any} */ (this.ext.settings).set_string(
          "workspace-skip-tile",
          skippedArr.toString()
        );
      ...
      case "LayoutStackedToggle":
        if (!focusNodeWindow) return;
        if (!(/** @type {any} */ (this.ext.settings).get_boolean("stacked-tiling-mode-enabled")))
          return;
      ...
      case "LayoutTabbedToggle":
        if (!focusNodeWindow) return;
        if (!(/** @type {any} */ (this.ext.settings).get_boolean("tabbed-tiling-mode-enabled")))
          return;
        ...
        if (focusNodeWindow) {
          ...
          switch (layoutDirection) {
            ...
            case "RIGHT":
              ...
              break;
            case "CENTER": {
              let metaRect = /** @type {any} */ (this.focusMetaWindow).get_frame_rect();
              layout.x = "center";
              layout.y = "center";
              layout = {
                x: Utils.resolveX(layout, /** @type {any} */ (this.focusMetaWindow)),
                y: Utils.resolveY(layout, /** @type {any} */ (this.focusMetaWindow)),
      ...
      case "ShowTabDecorationToggle": {
        ...
        let showTabs = /** @type {any} */ (this.ext.settings).get_boolean(
          "showtab-decoration-enabled"
        );
        /** @type {any} */ (this.ext.settings).set_boolean("showtab-decoration-enabled", !showTabs);
  ...
  get windowsActiveWorkspace() {
    ...
    return /** @type {any} */ (global.display).get_tab_list(
  ...
  get windowsAllWorkspaces() {
    ...
    for (let i = 0; i < wsManager.get_n_workspaces(); i++) {
      Array.prototype.push.apply(
        windowsAll,
        /** @type {any} */ (global.display).get_tab_list(
          Meta.TabList.NORMAL_ALL,
  ...
  determineSplitLayout() {
    ...
    let monitorRect = /** @type {any} */ (global.display).get_monitor_geometry(
  ...
  rectForMonitor(node, targetMonitor) {
    ...
    if (currentWorkArea && nextWorkArea) {
      ...
      let hRatio = nextWorkArea.height / currentWorkArea.height;
      let wRatio = nextWorkArea.width / currentWorkArea.width;
      rect.height *= hRatio;
      rect.width *= wRatio;
  ...
  renderTree(from, force = false) {
    ...
    if (
      this._freezeRender ||
      !(/** @type {any} */ (this.ext.settings).get_boolean("tiling-mode-enabled"))
    ) {
      this.updateDecorationLayout();
  ...
  showWindowBorders() {
    ...
    let splitBorderEnabled = /** @type {any} */ (this.ext.settings).get_boolean(
      "split-border-toggle"
    );
    let tilingModeEnabled = /** @type {any} */ (this.ext.settings).get_boolean(
      "tiling-mode-enabled"
  ...
  trackWindow(_display, metaWindow) {
    let autoSplit = /** @type {any} */ (this.ext.settings)?.get_boolean("auto-split-enabled");
    let focusMetaWindow = /** @type {any} */ (this.focusMetaWindow);
    if (autoSplit && focusMetaWindow) {
      let currentFocusNode = this.tree.findNode(focusMetaWindow);
      if (currentFocusNode) {
        ...
        if (layout === LAYOUT_TYPES.HSPLIT || layout === LAYOUT_TYPES.VSPLIT) {
          let frameRect = focusMetaWindow.get_frame_rect();
  ...
  isActiveWindowWorkspaceTiled(metaWindow) {
    ...
    let skipWs = /** @type {any} */ (this.ext.settings).get_string("workspace-skip-tile");
  ...
  windowDestroy(actor) {
    ...
    let nodeWindow = /** @type {any} */ (this.tree.findNodeByActor(actor));

    if (nodeWindow?.isWindow()) {
      this.tree.removeNode(nodeWindow);
      this.renderTree("window-destroy-quick", true);
      this.removeFloatOverride(/** @type {any} */ (nodeWindow.nodeValue), true);
    }
    ...
    monWsNoMaxWindows.forEach((monitorWs) => {
      ...
      activeMonWsCons.forEach((con) => {
        ...
        if (con.decoration && tiled.length > 0 && showTabs) {
          con.decoration.show();
          const focusMetaWindow = /** @type {any} */ (this.focusMetaWindow);
  ...
  movePointerWith(nodeWindow, { force = false } = {}) {
    ...
    const shouldWarp =
      force || /** @type {any} */ (this.ext.settings).get_boolean("move-pointer-focus-enabled");
    if (shouldWarp) {
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    if (nodeWinAtPointer) {
      ...
      const centerLayout = /** @type {any} */ (this.ext.settings)
      ...
      const updatePreview = (focusNodeWindow, previewParams) => {
        ...
        let previewHintEnabled = /** @type {any} */ (this.ext.settings).get_boolean(
      ...
      /** @type {{ className: string; targetRect: any }} */
      let previewParams = {
        className: "",
        targetRect: null,
      };
  ...
  _focusWindowUnderPointer() {
    ...
    let pointer = /** @type {[number, number]} */ (global.get_pointer());
  ...
  _handleResizing(focusNodeWindow) {
    ...
    for (let grabOp of grabOps) {
      ...
      let frameRect = /** @type {any} */ (this.focusMetaWindow).get_frame_rect();
  ...
  _liveResizeNeighbors(draggingNodeWindow) {
    const draggingMetaWin = /** @type {any} */ (draggingNodeWindow.nodeValue);
  ...
  _getDragDropCenterPreviewStyle() {
    const centerLayout = /** @type {any} */ (this.ext.settings).get_string("dnd-center-layout");
  ...
  reloadWindowOverrides() {
    ...
    const freshProps = /** @type {any} */ (this.ext.configMgr).windowProps;
    if (freshProps) {
